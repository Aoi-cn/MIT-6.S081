* 用户态的sysinfo()，首先系统会从user/user.h里找到声明，随后由链接到 usys.S 中的汇编代码来实现的。usys.S是通过usys.pl生成的。usys.S 文件定义了所有系统调用的入口点（如下），这些入口点使用 .global 指令使函数名在链接时可见。当用户程序中调用 sysinfo() 时，链接器会将调用解析为 usys.S 中的 sysinfo 函数。
```
.global sysinfo
sysinfo:
 li a7, SYS_sysinfo
